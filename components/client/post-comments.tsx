        ...
        <div className="grid grid-cols-1 gap-y-8 mb-6">
          {comments.length == 0 || comments[0].content ? (
            comments.map((comment) => (
              <div key={comment.slug.number} className="border-2 border-[#f4f1ea] dark:border-slate-500  rounded-xl">
          ...
          <div className="flex justify-center mb-10 mt-10">
            <Button className="hover:bg-[#9aae8e] dark:hover:bg-[#818CF8] dark:hover:text-white">
              <Link
                href = {`https://github.com/${username}/${REPO_NAME}/issues/${issuenumber}#issuecomment-new`}
                />
              Leave a new comment
            </Button>
          </div>
